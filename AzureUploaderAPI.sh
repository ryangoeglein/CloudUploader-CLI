...
# Function to prompt user to authenticate with Service Principal
authenticate_with_service_principal() {
    read -p "Enter your Service Principal App ID: " APP_ID
    read -sp "Enter your Service Principal Password: " PASSWORD
    echo
    read -p "Enter your Tenant ID: " TENANT_ID

    az login --service-principal -u "$APP_ID" -p "$PASSWORD" --tenant "$TENANT_ID"
    if [ $? -ne 0 ]; then
        echo "Azure authentication with Service Principal failed. Exiting."
        exit 1
    fi
}
